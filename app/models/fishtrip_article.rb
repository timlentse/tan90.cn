  ...
  def self.find_seo_article(params)
    ...
    self.where(params).each do |article|
      if articles[article.type_name]
        articles[article.type_name].push({:title=>article.title,:link=>"/articles/#{article.article_id}/"})
      else
        articles[article.type_name] = [{:title=>article.title,:link=>"/articles/#{article.article_id}/"}]
      end
    end
